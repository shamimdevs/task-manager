...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <form method="GET" action="{{ route('admin.users.index') }}" class="mb-6">
        <div class="flex gap-2 sm:gap-3">
            ...
            <button type="submit" class="btn-primary text-sm px-4 flex-shrink-0">Search</button>
            @if($search)
            <a href="{{ route('admin.users.index') }}"
               class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm text-slate-400 border border-slate-700 hover:text-slate-200 hover:border-slate-500 transition-all no-underline flex-shrink-0">
                ✕
            </a>
            @endif
        </div>
    ...
    <div class="flex flex-col gap-3 sm:hidden">
        ...
        <div class="glass-card p-4">
            ...
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400 to-violet-500 flex items-center justify-center font-bold text-sm text-white flex-shrink-0 overflow-hidden border border-slate-700">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5 flex-wrap">
                        <span class="text-sm font-semibold text-slate-100 truncate">{{ $user->name }}</span>
                        @if($user->id === auth()->id())
                ...
                <div>
                    ...
                    <form method="POST" action="{{ route('admin.users.toggle-status', $user) }}" class="inline">
                        ...
                        <button type="submit" class="flex items-center gap-2 cursor-pointer bg-transparent border-none p-0">
                            ...
                            <span class="font-medium {{ $user->isActive() ? 'text-emerald-400' : 'text-slate-500' }}">
                    ...
                    <div class="flex items-center gap-2 opacity-60">
                        ...
                        <span class="font-medium text-emerald-400">Active</span>
                    </div>
                    @endif
                </div>
            ...
            <div class="flex items-center gap-2 pt-3 border-t border-slate-800">
                <a href="{{ route('admin.users.report', $user) }}"
                   class="flex-1 text-center text-xs px-2.5 py-1.5 rounded-md border border-violet-400/30 bg-violet-400/8 text-violet-400 hover:bg-violet-400/20 transition-all no-underline">
                    Report
                </a>
                <a href="{{ route('admin.users.edit', $user) }}"
                   class="flex-1 text-center text-xs px-2.5 py-1.5 rounded-md border border-cyan-400/30 bg-cyan-400/8 text-cyan-400 hover:bg-cyan-400/20 transition-all no-underline">
                    Edit
                </a>
                @if($user->id !== auth()->id())
                <form method="POST" action="{{ route('admin.users.destroy', $user) }}"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                        class="w-full text-xs px-2.5 py-1.5 rounded-md border border-red-400/30 bg-red-400/8 text-red-400 hover:bg-red-400/20 transition-all cursor-pointer">
                        Delete
                    </button>
                </form>
                @endif
    ...
    <div class="hidden sm:block glass-card overflow-hidden p-0">
        <div class="overflow-x-auto">
            <table class="w-full border-collapse">
                <thead>
                    <tr class="border-b border-slate-800">
                        <th class="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">#</th>
                ...
                <tbody class="divide-y divide-slate-800">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="px-4 py-3.5 text-sm text-slate-400">{{ $user->email }}</td>
